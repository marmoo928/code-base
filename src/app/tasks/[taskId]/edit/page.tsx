...
export default function EditTaskPage({ params }: { params: Promise<{ taskId: string }> }) {
        ...
        const fetchTask = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    setXp(task.xp);
                    setTags(task.tags.map((t: any) => t.tag.name).join(", "));
                    setDescription(task.description);
                    setInputDescription(task.inputDescription);
                    setOutputDescription(task.outputDescription);
                    setExampleInput(task.exampleInput);
                    setExampleOutput(task.exampleOutput);
                    setConstraints(task.constraints);
                    setTestCases(task.testCases.map((tc: any) => ({
                        input: tc.input,
                        expectedOutput: tc.expectedOutput,
                        isHidden: tc.isHidden
                    })));
                    setPathwayId(task.pathwayId || "");
                } else {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-black flex items-center justify-center text-stone-500 font-bold">
                Loading task data...
            </div>
        );
    }
            ...
            <div className="max-w-4xl mx-auto bg-stone-900 border border-stone-800 rounded-3xl p-8 shadow-2xl">
                <div className="mb-8 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-stone-400">Update the programming challenge details</p>
                    </div>
                    <Link href="/tasks" className="text-stone-400 hover:text-white transition-colors">
                        Cancel
                    </Link>
                </div>
